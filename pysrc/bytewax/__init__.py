...
Adding descriptive step IDs to your steps is cruical because Bytewax's
built-in operators will try to label thrown exceptions with the step
they came from.

Because operators are defined hierarchically into nested **substeps**,
it's possible you'll see exceptions with step IDs that are dotted
paths similar to `branching_math.do_double.flat_map`. Don't fret! Read
them from left to right: the first component will be the flow ID, then
your top-level step ID.

Just because an exception references a substep of one of your step
IDs, does not mean the bug is in the inner step. It's possible some
logic function or value supplied is not conforming to the contract the
operator set.
